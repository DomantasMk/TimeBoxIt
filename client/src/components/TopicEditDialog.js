...
export default function TopicEditDialog({topic, openState, close}) {
    const [state, setState] = useState({ title: topic.title, description: topic.description});
  ...
  return (
    <div>
      <Dialog open={openState} onClose={handleClose} aria-labelledby="form-dialog-title">
        ...
        <DialogContent>
          <TextField autoFocus label="Title" fullWidth defaultValue={topic.title} onChange={handleChange} name="title"/>
          <TextField autoFocus label="Description" fullWidth defaultValue={topic.description} onChange={handleChange} name="description"/>

        </DialogContent>
        <DialogActions>
          <Button onClick={()=>{setDeleteTopic(true)}} color="primary">
            Delete
          </Button>
